...
class ProductUnitSerializer(serializers.ModelSerializer):
    ...
    batch = ProductBatchListSerializer(source="current_batch", read_only=True)
...
class ProductSerializer(serializers.ModelSerializer):
    units = ProductUnitSerializer(many=True, read_only=True, source="productunit_set")
    class Meta:
        model = Product
        exclude = ("deleted_at", "restored_at", )
...
class CreateProductBatchSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ProductBatch
        fields = ("unit", "product", "quantity", "cost_price", "selling_price")
    ...
    def validate(self, attrs):
        ...
        if not self.instance:
            attrs["product_unit"] = ProductUnit.objects.get_or_create(product=attrs.pop("product"), unit=attrs.pop("unit"))[0]
...
class CreateSaleTransactionSerializer(serializers.ModelSerializer):
    ...
    percentage_discount = serializers.DecimalField(max_value=100, min_value=0, decimal_places=1, max_digits=3, required=False)
    class Meta:
